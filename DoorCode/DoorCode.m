...
kMax = 10; %number of digits on doorcode
...
for k = 2:kMax
possibleCodes = dec2base(0:k^n-1, k); %all possible codes
%but we only want the ones containing a k
kCodes = string(possibleCodes(contains(string(possibleCodes), num2str(k-1)), :));

%we need to reserve codes like '222', '220' and '200'
for j=0:n-1
    kCodes(strcmp(kCodes, string([repmat(num2str(k-1), 1, n-j) repmat('0', 1, j)]))) = [];
end
...
pushed = [num2str(k-1) (repmat('0', 1, n-1))];
